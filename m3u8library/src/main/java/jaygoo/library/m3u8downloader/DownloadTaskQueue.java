...
class DownloadTaskQueue {
     ...
     * @param task
     */
    public void offer(M3U8DownloadTask task){
        if (task!=null){
            queue.add(task);
        }
    }
     ...
     * @return 是否成功移除
     */
    public boolean remove(M3U8DownloadTask task){
        if (contains(task)){
     ...
     * @return
     */
    public boolean contains(M3U8DownloadTask task){
     ...
     * @return
     */
    public M3U8DownloadTask getTask(String url){
        try {
            for (int i = 0; i < queue.size(); i++){
                if (queue.get(i).getTaskId().equals(MD5Utils.encode(url))){
                    return queue.get(i);
                }
            }
        }catch (Exception e){
