...
public class DownloadPresenter {
    ...
    public static void addM3u8Task(Context context, String url, String name, String imgUrl) {
        if (url.startsWith("http")) {
            if( url.contains(".m3u8") || url.contains("/m3u8?")){
                ...
                Toast.makeText(context, "此源无法下载,请更换源！", Toast.LENGTH_SHORT).show();
                return;
            }
        }else {
     ...
     * @param url
     */
    public static List<M3u8DownloadingInfo> getM3u8DownLoading(String url) {
        DowningDao downingDao = M3U8dbManager.getInstance(M3U8Library.getContext()).downingDao();
        String taskId = MD5Utils.encode(url);
        return downingDao.getById(taskId);
    }
    /**
     * 检查是否本地已经下载过或在下载中
     *
     * @param url
     */
    public static List<M3u8DoneInfo> getM3u8Done(String url) {
        DoneDao dao2 = M3U8dbManager.getInstance(M3U8Library.getContext()).doneDao();
        String taskId = MD5Utils.encode(url);
        return dao2.getById(taskId);
    }
